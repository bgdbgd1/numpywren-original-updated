...
async def reset_msg_visibility(msg, queue_url, loop, timeout, timeout_jitter, lock):
    ...
    while(lock[0] == 1 and loop.is_running()):
        try:
            ...
            async with boto3.client('sqs', use_ssl=False,  region_name="us-west-2") as sqs_client:
    ...
    try:
        while(loop.is_running()):
            ...
            for queue_url in program.queue_urls[::-1]:
                async with boto3.client('sqs', use_ssl=False,  region_name=program.control_plane.region) as sqs_client:
