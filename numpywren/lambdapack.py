...
def put(key, value, s3=True, s3_bucket="bogdan-pywren-48"):
    print(key, value)
    if (s3):
...
def get(key, s3=True, s3_bucket="bogdan-pywren-48"):
...
def incr(key, amount=1, s3=True, s3_bucket="bogdan-pywren-48"):
    if (s3):
        # read from S3
...
def decr(key, amount, s3=True, s3_bucket="bogdan-pywren-48"):
    if (s3):
        # read from S3
        response = get(key, s3_bucket=s3_bucket)
        response = int(response)
...
class LambdaPackProgram(object):
    ...
    def __init__(self, program, config, num_priorities=1, eager=False, block_sparse=False):
        ...
        put(self.hash, PS.NOT_STARTED.value, s3_bucket=self.config['s3']['bucket'])
    ...
    def start(self, parallel=False):
        put(self.hash, PS.RUNNING.value)
    ...
    def return_success(self):
        # print("RETURNING....")
        put(self.hash, PS.SUCCESS.value)
    ...
    def get_flops(self):
        return get("{0}_flops".format(self.hash))

    def get_read(self):
        return get("{0}_read".format(self.hash))

    def get_write(self):
        return get("{0}_write".format(self.hash))
